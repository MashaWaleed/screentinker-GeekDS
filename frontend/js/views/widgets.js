...
function tzOptions(selected) {
  ...
  return all.map((z) => `<option value="${escAttr(z)}" ${z === selected ? 'selected' : ''}>${escAttr(z)}</option>`).join('');
}
/*
...
export async function render(container) {
    ...
    switch (type) {
      case 'clock':
        ...
        html += `
          ...
          <div class="form-group"><label>${t('widget.field.timezone')}</label><select id="wTimezone" class="input" style="background:var(--bg-input)">${tzOptions(timezone)}</select><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px"><button type="button" class="btn btn-secondary btn-sm" id="wUseDashboardTimezone">${t('widget.field.use_dashboard_timezone')}</button><button type="button" class="btn btn-secondary btn-sm" id="wUseServerTimezone">${t('widget.field.use_server_timezone')}</button></div><div class="form-hint" id="wTimezoneHint" style="font-size:12px;color:var(--text-muted);margin-top:4px">${t('widget.field.timezone_hint')}</div></div>
          ...
          <div class="form-group"><label>${t('widget.field.date_format')}</label><select id="wDateFormat" class="input" style="background:var(--bg-input)"><option value="full" ${dateFormat === 'full' ? 'selected' : ''}>${t('widget.field.date_format_full')}</option><option value="long" ${dateFormat === 'long' ? 'selected' : ''}>${t('widget.field.date_format_long')}</option><option value="medium" ${dateFormat === 'medium' ? 'selected' : ''}>${t('widget.field.date_format_medium')}</option><option value="short" ${dateFormat === 'short' ? 'selected' : ''}>${t('widget.field.date_format_short')}</option></select></div>
  ...
  function initClockForm() {
    ...
    const setTimezone = (zone) => {
      ...
      if (!Array.from(timezone.options).some((option) => option.value === zone)) {
  ...
  function getConfigFromForm(type) {
    ...
    switch (type) {
      case 'clock': Object.assign(config, { format: val('wFormat'), timezone: val('wTimezone'), font_size: parseInt(val('wFontSize')) || 64, color: val('wColor'), background: val('wBg'),
        ...
        date_font_size: parseInt(val('wDateFontSize')) || 21,
